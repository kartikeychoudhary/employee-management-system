...
export class SearchEmployeeComponent implements OnInit {
  ...
  returnSearchInput() {
    if (this.searchKey === 'dojR' || this.searchKey === 'skills' || this.searchKey === 'salaryR') {
      return false;
    } else {
      return true;
    }
  ...
  salaryRange(){
    ...
    this.employees.forEach((res) => {
      var sal = parseInt(res['salary']);

      if (sal >=  this.minValue && sal <= this.maxValue) {
        temp.push(res);
        count += 1;
      }
    });
  ...
  Search() {
    if (this.search == '') {
      ...
      this.employees = this.employees.filter((res) => {
        return res[this.searchKey]
          .toString()
          .toLocaleLowerCase()
          .match(this.search.toString().toLocaleLowerCase());
      });
    }
  }
